...
public class Chunk
{
    ...
    public (Vector3[], int[]) CalculateVerticesAndTriangles(int triangleOffset)
    {
        ...
        int borderedSize = resolution + 2; // for border vertices for normals (border to get rid of lines between quads in lighting)
        Vector3[] vertices = new Vector3[resolution * resolution];
        int[] triangles = new int[(resolution-1) * (resolution-1) * 6];
        int triIndex = 0;

        //int i = 0; //replacing as he does with vertexIndex
        ...
        for (int y = 0; y < borderedSize; y++)
        {
            for (int x = 0; x < borderedSize; x++)
            {
                ...
                //vertices[vertexIndex] = pointOnUnitSphere * Planet.size * ((elevation * 0.5f) + 0.5f); //Mathf.Max(elevation, 1f);
                //vertices[vertexIndex] = pointOnUnitSphere * Planet.size * (1 + elevation); // the way seblague did it

                elevation = Planet.size * (1 + elevation);
                planet.elevationMinMax.AddValue(elevation);
                vertices[vertexIndex] = pointOnUnitSphere * elevation;    // modified for shader stuff

                if (x != (borderedSize-1) && y != (borderedSize-1))
                {
                    //triangles[triIndex + 0] = vertexIndex + triangleOffset;
                    //triangles[triIndex + 1] = vertexIndex + resolution + 1 + triangleOffset;
                    //triangles[triIndex + 2] = vertexIndex + resolution + triangleOffset;

                    //triangles[triIndex + 3] = vertexIndex + triangleOffset;
                    //triangles[triIndex + 4] = vertexIndex + 1 + triangleOffset;
                    //triangles[triIndex + 5] = vertexIndex + resolution + 1 + triangleOffset;

                    triangles[triIndex + 0] = vertexIndicesMap[x,y] + triangleOffset;   // adc
                    triangles[triIndex + 1] = vertexIndicesMap[x+1,y+1] + triangleOffset;
                    triangles[triIndex + 2] = vertexIndicesMap[x,y+1] + triangleOffset;

                    triangles[triIndex + 3] = vertexIndicesMap[x,y] + triangleOffset;   // dab changed to abd
                    triangles[triIndex + 4] = vertexIndicesMap[x+1,y] + triangleOffset;
                    triangles[triIndex + 5] = vertexIndicesMap[x+1,y+1] + triangleOffset;
        ...
        for (int j = 0; j < borderVertices.Length; j++)
        {
            ...
            elevation = Planet.size * (1 + elevation);
            planet.elevationMinMax.AddValue(elevation);
            borderVertices[j] = pointOnUnitSphere * elevation;    // modified for shader stuff
        }
...
// NEW TERRAINFACE AFTER IMPLEMENTING MESHSIMPLIFICATIONINCREMENT AND VERTICESPERLINE (BROKEN)
/*
using System.Collections;
using System.Collections.Generic;
using UnityEngine;
...
public class Chunk
{
    ...
    public (Vector3[], int[], Vector3[], int[]) CalculateVerticesAndTriangles(int triangleOffset)
    {
        ...
        int[] triangles = new int[(verticesPerLine-1) * (verticesPerLine-1) * 6]; // was breaking less when all 4 meshSize were borderedSize but seems fine now
        int triIndex = 0;

        Vector3[] borderVertices = new Vector3[verticesPerLine * 4 + 4];
        int[] borderTriangles = new int[verticesPerLine * 4 * 6];
        ...
        for (int y = 0; y < borderedSize; y += meshSimplificationIncrement)
        {
            for (int x = 0; x < borderedSize; x += meshSimplificationIncrement)
            {
                ...
                Vector2 percent = new Vector2(x - meshSimplificationIncrement, y - meshSimplificationIncrement) / (borderedSize-1);
...
// LATEST BIT I GOT TO TRYING TO IMPLEMENT BORDER V+TS (BROKEN)
...
public class Chunk
{
    ...
    public (Vector3[], int[], Vector3[], int[]) CalculateVerticesAndTriangles(int triangleOffset)
    {
        ...
        // THIS BIT SHOULD BE COMMENTED OUT
